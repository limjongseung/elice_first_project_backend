...
const { userCreateValidation, userUpdateValidation } = require('./account-Validation');

// 로그인한 유저의 정보를 리턴
router.get('/', passport.authenticate('jwt-user', { session: false }), async (req, res, next) => {
  try {
    // 유저 정보를 이메일로 검색
 ...
 */
router.put('/',passport.authenticate('jwt-user',{ session: false } ), userUpdateValidation, async (req, res , next) => {
  try {
  const data = await search.UserSearch("email",req.user.email)
 ...
 */
router.post('/login',passport.authenticate('local-user',{ session: false } ), async(req,res, next) => {
  try {
